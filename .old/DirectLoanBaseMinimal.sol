 ...
 *
 */
abstract contract DirectLoanBaseMinimal is IDirectLoanBase, IPermittedERC20s, BaseLoan, NFTReceiver, ILoanData {
...
//        address revenueSharePartner,
    ...
    function _createLoan(
        ...
        return _createLoanNoNftTransfer(_loanType, _loanTerms, //_loanExtras, 
        _borrower, _lender, _referrer);
    }
...
//        loanIdToLoanExtras[loanId] = _loanExtras;
    ...
    function _transferNFT(
        ...
        Address.functionDelegateCall(
            _loanTerms.nftCollateralWrapper, // address 
...
//            loanExtras.revenueSharePartner, // this could be a non address zero even if revenueShare is 0
     ...
     */
    /*
    function _loanSanityChecks(Vault _vault, address _nftWrapper) internal view {
        if (!getERC20Permit(_vault.getLoanERC20Denomination()) {
        ...
        // @todo 
        /*
        if (uint256(_vault.loanDuration) > maximumLoanDuration) {
            revert LoanDurationExceedsMaximum();
        }
        if (uint256(_vault.loanDuration) == 0) {
            revert LoanDurationCannotBeZero();
        }*/ 

       // @todo ? 
       /* 
        if (_vault.loanAdminFeeInBasisPoints != adminFeeInBasisPoints) {
